...
class GNNMOT(nn.Module):
    ...
    def forward(
        ...
        valid_regr_affinity_matrix = torch.mul(init_affinity_matrix, regr_affinity_matrix)
        ...
        assert init_affinity_matrix.shape == regr_affinity_matrix.shape
        if self.mode == 'train':
            loss_dict = self._compute_layer_loss(h, regr_affinity_matrix, gt_affinity_matrix)
            return loss_dict['total_loss'], loss_dict['aff_loss'], loss_dict['triplet_loss'],valid_regr_affinity_matrix
        else:
            # matching assingment
            matched_indices = []
            ...
            min_indicies = torch.min(valid_regr_affinity_matrix, dim = 1)
            for i in range(N):
                print(valid_regr_affinity_matrix[i])
                j = valid_regr_affinity_matrix[i].argmin()
