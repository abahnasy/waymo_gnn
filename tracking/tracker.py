  ...
  def step_centertrack(self, results, time_lag):
    if len(results) == 0:
      ...
      for det in results:
        ...
        det['ct'] = np.array(det['translation'][:2]) # xy global coordinates
        det['tracking'] = np.array(det['velocity'][:2]) * -1 *  time_lag # previous positions wrt to time_lag
    ...
    if 'tracking' in results[0]:
      ...
      [ det['ct'] + det['tracking'].astype(np.float32)
       for det in results], np.float32) # get estimated previous positions [500,2]
    else:
      dets = np.array(
